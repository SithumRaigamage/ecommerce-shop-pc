    ...
    <div>
      ...
      {orderItems.length === 0 ? (
        <div className="py-16 text-center">
          ...
          <p className="text-muted-foreground mt-2">Add some products to your cart first.</p>
          ...
          <ul className="flex flex-col gap-6">
            {orderItems.map((item) => (
              <li key={item.product.id}>
                <Card>
                  <CardContent className="grid gap-6 sm:grid-cols-[150px_1fr]">
                    <ProductImage
                    ...
                    <div className="flex flex-col justify-center gap-1">
                      ...
                      <p className="text-primary text-xl">{formatLKR(item.product.price)}</p>
                      {item.color && (
                        <p className="text-muted-foreground text-sm">Color: {item.color}</p>
                      )}
                      {item.size?.name && (
                        <p className="text-muted-foreground text-sm">Size: {item.size.name}</p>
                      )}
          ...
          <Card className="h-fit">
            ...
            <CardContent>
              <dl className="space-y-3">
                <div className="text-muted-foreground grid grid-cols-3 text-sm font-medium">
                ...
                <div className="flex justify-between">
                  <dt>Shipping Fee</dt>
                  <dd className="font-medium">{formatLKR(shippingFee)}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt>Tax</dt>
                  <dd className="font-medium">{formatLKR(tax)}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt>Coupon</dt>
                  <dd className="font-medium">{appliedCoupon || '—'}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt>Discount</dt>
